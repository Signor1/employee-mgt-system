use crate::contract_import::token_contract::Client as TokenClient;
...
impl EmployeeManagementContract {
    ...
    pub fn promote_employee(
        ...
        if employee_data.rank.to_u32() == new_rank.to_u32() {
            return Err(ContractError::SameRank);
        }
    ...
    pub fn suspend_employee(
        ...
        match employee_data.status {
            EmployeeStatus::Suspended => return Err(ContractError::EmployeeAlreadySuspended),
    ...
    pub fn reactivate_employee(
        ...
        match employee_data.status {
            EmployeeStatus::Active => return Err(ContractError::EmployeeAlreadyActive),
    ...
    pub fn pay_salary(env: Env, admin: Address, employee: Address) -> Result<(), ContractError> {
        Self::ensure_initialized(&env)?;
        Self::ensure_admin(&env, &admin)?;
        ...
        if employee_data.status == EmployeeStatus::Suspended {
            return Err(ContractError::EmployeeSuspended);
        }
        ...
        let token_contract: Address = env
            ...
            .ok_or(ContractError::TokenContractError)?;
    ...
    // Get methods
    pub fn get_employee(env: Env, employee: Address) -> Result<Employee, ContractError> {
        Self::ensure_initialized(&env)?;
    ...
    pub fn get_all_employees(env: Env) -> Result<Vec<Employee>, ContractError> {
        Self::ensure_initialized(&env)?;
    ...
    pub fn get_institution_info(env: Env) -> Result<InstitutionInfo, ContractError> {
        Self::ensure_initialized(&env)?;
    ...
    pub fn pay_all_salaries(env: Env, admin: Address) -> Result<(), ContractError> {
        Self::ensure_initialized(&env)?;
        Self::ensure_admin(&env, &admin)?;
    ...
    fn ensure_employee_exists(env: &Env, employee: &Address) -> Result<(), ContractError> {
        let employee_key = DataKey::Employee(employee.clone());
        if !env.storage().persistent().has(&employee_key) {
    ...
    fn update_total_employees(env: &Env) -> Result<(), ContractError> {
        let employee_list: Vec<Address> = env
            .storage()
            .persistent()
            .get(&DataKey::EmployeeList)
